...
{
	if (mrq->sbc) {
		pr_debug("<%s: starting sbc CMD%u arg %08x flags %08x>\n",
			 mmc_hostname(host), mrq->sbc->opcode,
...
void mmc_set_data_timeout(struct mmc_data *data, const struct mmc_card *card)
{
	...
	if (mmc_card_long_read_time(card) && data->flags & MMC_DATA_READ) {
		pr_debug("mmc-core: card_long_reead_time detected, setting timeout to 600ms.\n");
		//turn this on either way?
	...
	if (mmc_host_is_spi(card->host)) {
		pr_debug("mmc-core: Card is in SPI mode, setting high timeout in mmc_set_data_tiemout.\n");
		if (data->flags & MMC_DATA_WRITE) {
			pr_info("mmc-core: data write occurring, setting timeout to 1s.\n");
			if (data->timeout_ns < 1000000000)
				data->timeout_ns = 1000000000;	/* 1s */
		} else {
			pr_info("mmc-core: data write not occurring, setting timeout to 100ms.\n");	
